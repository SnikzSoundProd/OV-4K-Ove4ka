...
struct Config {
    int vocab_size = 0;
    int d_model = 384;    // ← вернули на 384
    int n_heads = 32;     // ← 32 головы как было
    int d_qk = 2;         // ← 2D Q/K как было
    int d_v = 12;         // ← 12 на значение как было
    int n_layers = 8;
    int d_ff = 1536;      // ← вернули FF на 1536
    int seq_len = 512;
    int batch_size = 4;
    int micro_batch = 2;
    int top_k_attn = 12;
...
// Читаем весь текст и режем его на чанки по seq_len токенов
std::vector<EncodedSample> prepare_pretrain_samples(const std::vector<int>& corpus_tokens, int seq_len) {
    std::vector<EncodedSample> samples;
    int S = seq_len;
    
    // Шаг сдвига — seq_len. Читаем текст блоками.
    for (size_t i = 0; i + S < corpus_tokens.size(); i += S) {
        std::vector<int> chunk(corpus_tokens.begin() + i, corpus_tokens.begin() + i + S + 1);
        
        // prompt_len ставим 0, так как у нас нет "вопроса", учим с самого начала чанка
        samples.push_back({chunk, 0}); 
    }
    return samples;
}
...
void train(const std::string& data, const std::string& mdl, int epochs) {
    // 1. Читаем файл один раз в переменную text
    std::string text = read_file(data);
    if (text.empty()) {
    ...
    // 2. Строим токенизатор по всему тексту (для претрейна лучше 4096 вокаб)
    Tokenizer tok;
    tok.build(text, 4096); 

    // 3. Кодируем весь текст в токены (вот теперь 'corpus_ids' создана!)
    auto corpus_ids = tok.encode(text);
    printf("[Data] BPE tokens: %zu\n", corpus_ids.size());

    // 4. Создаем конфиг (теперь 'cfg' создана!)
    Config cfg;
    cfg.vocab_size = tok.vocab_size();

    // 5. Нарезаем текст на чанки по seq_len (используем то, что создали выше)
    auto samples = prepare_pretrain_samples(corpus_ids, cfg.seq_len);
    
    // Проверяем samples, а не pairs!
    if (samples.empty()) {
        fprintf(stderr, "[Data] No pre-training samples generated\n");
        return;
    }
    printf("[Data] Pre-training samples: %zu\n", samples.size());
...
// --- Парсер диалогов Человек/Бот ---
// struct DialoguePair { std::string user, bot; };

static inline bool starts_with_fix(const std::string& s, const std::string& prefix) {
    return s.size() >= prefix.size() && s.compare(0, prefix.size(), prefix) == 0;
}

static inline std::string trim_copy_fix(std::string s) {
    auto is_ws = [](unsigned char c){ return c==' ' || c=='\t' || c=='\r' || c=='\n'; };
    while (!s.empty() && is_ws((unsigned char)s.back())) s.pop_back();
    size_t i = 0;
    while (i < s.size() && is_ws((unsigned char)s[i])) ++i;
...
std::vector<DialoguePair> extract_dialogue_pairs(const std::string& text) {
    ...
    while (std::getline(iss, line)) {
        auto s = trim_copy_fix(line);
        if (starts_with_fix(s, "Человек:")) {
            pending_user = trim_copy_fix(s.substr(std::string("Человек:").size()));
        } else if (!pending_user.empty() && starts_with_fix(s, "Бот:")) {
            std::string bot = trim_copy_fix(s.substr(std::string("Бот:").size()));
...
// ============================================================
//  ФИНАЛЬНАЯ ИСПРАВЛЕННАЯ РЕАЛИЗАЦИЯ FINETUNE
// ============================================================

struct LocalDialoguePair { std::string user, bot; };

static inline std::string local_trim(std::string s) {
    auto is_ws = [](unsigned char c){ return c==' ' || c=='\t' || c=='\r' || c=='\n'; };
    while (!s.empty() && is_ws((unsigned char)s.back())) s.pop_back();
    size_t i = 0;
    while (i < s.size() && is_ws((unsigned char)s[i])) ++i;
...
void finetune(const std::string& data_path, const std::string& base_mdl, const std::string& out_mdl, int epochs) {
    std::string raw = read_file(data_path);
    if (raw.empty()) {
        fprintf(stderr, "File not found or empty: %s\n", data_path.c_str());
        return;
    }

    // 1. Парсим диалоги Человек / Бот
    std::istringstream iss(raw);
    std::string line, pending_user;
    std::vector<LocalDialoguePair> pairs;

    while (std::getline(iss, line)) {
        auto s = local_trim(line);
        if (s.rfind("Человек:", 0) == 0) {
            pending_user = local_trim(s.substr(8));
        } else if (!pending_user.empty() && s.rfind("Бот:", 0) == 0) {
            std::string bot = local_trim(s.substr(4));
            if (!bot.empty()) pairs.push_back({pending_user, bot});
            pending_user.clear();
        } else if (!s.empty()) {
            pending_user.clear();
        }
    ...
    if (pairs.empty()) {
        fprintf(stderr, "[Finetune] Не найдено диалогов формата 'Человек:' / 'Бот:'\n");
        return;
    }
    ...
    // 2. Загружаем базовую модель (которую учили на Википедии)
    if (!model.load(base_mdl, tok)) {
        fprintf(stderr, "[Finetune] Не удалось загрузить базу: %s\n", base_mdl.c_str());
        return;
    }

    // Параметры для стабильности дообучения (уменьшаем LR)
    model.cfg.lr = std::min(model.cfg.lr, 2e-5f); 
    model.cfg.micro_batch = std::max(model.cfg.micro_batch, 4);

    const int S = model.cfg.seq_len;
    std::vector<EncodedSample> samples; // Используем глобальный EncodedSample
    ...
    for (const auto& p : pairs) {
        ...
        if ((int)prompt_ids.size() >= S) continue;
        if ((int)full_ids.size() > S + 1) full_ids.resize(S + 1);
        if ((int)full_ids.size() <= (int)prompt_ids.size()) continue;

        if (full_ids.back() != tok.eos_id && (int)full_ids.size() < S + 1) {
            full_ids.push_back(tok.eos_id);
        }
    ...
    if (samples.empty()) {
        fprintf(stderr, "[Finetune] После токенизации не осталось валидных сэмплов.\n");
        return;
    }

    printf("[Finetune] Загружена база: %s. Сэмплов для файнтюна: %zu\n", base_mdl.c_str(), samples.size());
    ...
    for (int ep = 0; ep < epochs; ep++) {
        ...
        for (int st = 0; st < steps; st++) {
            ...
            // accumulate_gradients_masked считает лосс только по ответам Бота!
            tot += model.accumulate_gradients_masked(sm.ids, sm.prompt_len, 1.0f / std::max(1, model.cfg.micro_batch));
            accum++;

            if (accum >= model.cfg.micro_batch || st + 1 == steps) {
                model.optimizer_step(1.0f);
                model.zero_grad();
                accum = 0;
            }

            if ((st + 1) % 50 == 0 || st + 1 == steps) {
                CUDA_CHECK(cudaDeviceSynchronize());
                auto t1 = std::chrono::steady_clock::now();
                int ms = (int)std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
                printf("  Finetune Epoch %d/%d step %d/%d loss=%.4f time=%dms\n",
                       ep + 1, epochs, st + 1, steps, tot / (st + 1), ms);
                t0 = t1;
            }
        }

        model.save(out_mdl, tok);
        std::string ckpt = checkpoint_path_for_epoch(out_mdl, ep + 1);
        model.save(ckpt, tok);
        printf("[Finetune] Сохранен чекпоинт: %s\n", ckpt.c_str());

        auto si = tok.encode("Человек: привет\nБот: ");
        auto gi = model.generate(si, 40, 0.55f, 0.78f, 1.24f, stop_seqs, tok.eos_id, 6);
        std::vector<int> tail(gi.begin() + si.size(), gi.end());
        printf("\n  [Finetune Gen] Q: привет\n  A: %s\n\n", cut_at_stop_markers(tok.decode(tail)).c_str());
    }
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 2) {
        ...
        printf("  %s finetune patch.txt base_model.bin out_model.bin [epochs=1]\n", argv[0]);
    ...
    if (mode == "train") {
        ...
        if (argc < 5) {
            fprintf(stderr, "Usage: %s finetune patch.txt base_model.bin out_model.bin [epochs=1]\n", argv[0]);
            cublasDestroy(cublas);
            return 1;
        }
        finetune(argv[2], argv[3], argv[4], (argc >= 6) ? std::stoi(argv[5]) : 1);
    } else if (mode == "chat") {
        if (argc < 3) {
